...
struct QuantileBFloat16HistogramBuckets
{
    ...
    void merge(const QuantileBFloat16HistogramBuckets & rhs)
    {
...
private:
    ...
    T getImpl(Float64 level) const
    {
        ...
        Pair *previous_pair = nullptr;
        Pair *threshold_pair = nullptr;
        ...
        for (const Pair * p = array; p != (array + size); ++p)
        {
            ...
            {
                threshold_pair = const_cast<Pair *>(p);
                break;
            }

            previous_pair = const_cast<Pair *>(p);    
        }

        if (threshold_pair == nullptr)
            return static_cast<T>(array[size - 1].first);
        else
        {
            if (previous_pair == nullptr)
                return static_cast<T>(threshold_pair->first);
            else
            {
                Float64 previous_accumulated = accumulated - threshold_pair->second;
                Float64 previous_threshold = threshold - previous_accumulated;
                Float64 previous_value = previous_pair->first;
                Float64 threshold_value = threshold_pair->first;

                return static_cast<T>(previous_value + (threshold_value - previous_value) * previous_threshold / threshold_pair->second);
            }
        }
    }
